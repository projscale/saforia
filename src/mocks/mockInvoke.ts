...
async function generate(master: string, postfix: string, methodId: string): Promise<string> {
  ...
  if (methodId === 'legacy_v2') {
    const d = await sha256(new TextEncoder().encode(master + postfix))
    return b64url(d)
  }
...
export async function mockInvoke<T = any>(cmd: string, args: any = {}): Promise<T> {
  switch (cmd) {
    case 'has_master': return true as T
    case 'setup_set_master': state.master = args.masterPassword || state.master; return (undefined as unknown) as T
