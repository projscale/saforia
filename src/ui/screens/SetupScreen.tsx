  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (valid && !busy) onSubmit() }} className="col">
        ...
        <PasswordInput label={t('masterPassword')} value={state.master} onChange={v => setState({ ...state, master: v })} placeholder="Strong master" autoComplete="new-password" />
        <div className="input-group">
          <PasswordInput label={t('confirmMaster')} value={state.master2} onChange={v => setState({ ...state, master2: v })} placeholder="Repeat master" autoComplete="new-password" describedBy={masterMismatch ? masterErrId : undefined} />
        ...
        <PasswordInput label={t('viewerPassword')} value={state.viewer} onChange={v => setState({ ...state, viewer: v })} placeholder="Device-only viewer" autoComplete="new-password" describedBy={viewerHelpId} />
        <div className="input-group">
          <PasswordInput label={t('confirmViewer')} value={state.viewer2} onChange={v => setState({ ...state, viewer2: v })} placeholder="Repeat viewer" autoComplete="new-password" describedBy={viewerMismatch ? viewerErrId : undefined} />
        ...
        <div className="row">
          <button className="btn primary" disabled={busy || !valid} aria-busy={busy ? 'true' : 'false'}>
            {busy ? (<span style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}><span className="spinner" aria-hidden="true"></span> Saving…</span>) : t('saveMaster')}
          </button>
        </div>
