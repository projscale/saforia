...
const bundle: Bundle = {
  en: {
    ...
    aboutBackupExport: 'Export a backup to a .safe file that contains only your saved entries (labels, postfixes, methods, timestamps, master fingerprints), never your master or viewer passwords. If you set a passphrase, the JSON inside is encrypted using Argon2id + ChaCha20‑Poly1305; without a passphrase it is plain JSON, so prefer a strong passphrase and store the file somewhere protected (encrypted disk, secure cloud, USB).',
    aboutBackupImport: 'Import a .safe file on the Backup tab to restore your Saved list on this device. Entries are tied to the master fingerprint they were created with, so set up the same master password first; then choose whether to merge with existing entries or enable Overwrite to fully replace them.',
  ...
  ru: {
    ...
    aboutBackupExport: 'Экспорт бэкапа в файл .safe, который содержит только сохранённые записи (метки, постфиксы, методы, время создания и отпечатки мастера), но не содержит сам мастер‑пароль или viewer‑пароль. При задании пароля архив шифруется (Argon2id + ChaCha20‑Poly1305 поверх JSON); без пароля внутри лежит открытый JSON, поэтому обычно рекомендуется ставить сильный пароль и хранить файл в защищённом месте (зашифрованный диск, «безопасное» облако, флешка).',
  ...
  zh: {
    ...
    aboutBackupExport: '将备份导出为 .safe 文件，其中只包含已保存条目（标签、后缀、方法、创建时间以及主密码指纹），不会包含主密码或 Viewer 密码本身。若设置口令，内部 JSON 会通过 Argon2id + ChaCha20‑Poly1305 加密；如果不设口令则为明文 JSON，因此推荐使用强口令，并将文件保存在受保护的位置（加密磁盘、安全云盘、U 盘等）。',
    aboutBackupImport: '在“备份”页从 .safe 文件导入，在本设备上恢复已保存列表。条目会绑定到它们创建时对应的主密码指纹，因此请先在新设备上配置相同的主密码；之后可以选择与现有条目合并，或开启“覆盖”以完全替换当前列表。',
