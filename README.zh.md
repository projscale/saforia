...
- 备份：通过 `.safe` 文件导出/导入已保存条目，文件中只包含标签、后缀、方法、创建时间和主密码指纹（不包含主密码或 Viewer 密码）。若设置口令，内部 JSON 会通过 Argon2id + ChaCha20‑Poly1305 加密；若不设置则为明文 JSON，建议使用强口令并将文件保存在加密磁盘或其他受保护的位置。
...
## 备份与 .safe 归档
- 导出：将备份导出为 `.safe` 文件，其中只包含已保存条目（标签、后缀、方法、创建时间以及主密码指纹），不会包含主密码或 Viewer 密码本身。若设置口令，内部 JSON 会通过 Argon2id + ChaCha20‑Poly1305 加密；若不设置则为明文 JSON，建议使用强口令并将文件保存在加密磁盘或其他受保护的位置（例如安全云盘或 U 盘）。
- 导入：在“备份”页从 `.safe` 文件导入，在本设备上恢复已保存列表。条目会绑定到它们创建时对应的主密码指纹，因此请先在新设备上配置相同的主密码，然后选择与现有条目合并，或启用“覆盖”以完全替换当前列表。

## CSV 备份与导入
- 导出 CSV：每条记录一行，列为 `fingerprint,label,postfix,method_id,created_at,id`（明文，无加密）。
- 预览导入：显示导入指纹及数量；将本地指纹拖到导入目标上进行映射，或拖到 “Ignore” 忽略。
- 应用导入：忽略的不会导入；其余按映射目标导入（Overwrite 完全替换当前列表，否则追加合并）。CSV 文件中包含标签和后缀，请将其视为敏感数据，仅在迁移期间短暂保存，并在完成后删除。
